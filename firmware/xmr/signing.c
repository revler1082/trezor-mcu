...
static bool xmr_signing = false;
static uint32_t data_total, data_left;
static XmrTxRequest resp;
...
static void send_request_chunk(void)
{
	...
	msg_write(MessageType_MessageType_MoneroTxRequest, &resp);
}
...
static void layoutMoneroConfirmTx(const uint8_t *to, uint32_t to_len, const uint8_t *value, uint32_t value_len)
{
	bignum256 val;
	uint8_t pad_val[32];
...
static void layoutMoneroData(const uint8_t *data, uint32_t len, uint32_t total_len)
{
	char hexdata[3][17];
	char summary[20];
	int i;
...
void monero_signing_init(MoneroSignTx *msg, const HDNode *node)
{
	monero_signing = true;
...
void monero_signing_abort(void)
{
	if (monero_signing) {
		memset(privkey, 0, sizeof(privkey));
		layoutHome();
		monero_signing = false;
	}
}
